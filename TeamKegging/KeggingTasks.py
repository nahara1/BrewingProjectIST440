    ...
    def Keggingtasksmain(self):  # kegging task start
        ...
        try:
            ...
            while True:
                ...
                if confirm_batch_id in ['Y', 'y', 'yes', 'Yes', 'YES']:
                    self.keg_log(batch_id, "Kegging",
                                 "Kegging Tasks: Batch ID entered as: " + batch_id)  # logging to service now
                    break
                else:
                    print("Confirmation failed. Please try again.")
                    self.keg_log(batch_id, "Kegging",
                                 "Kegging Tasks: Batch ID Confirmation Mismatch")  # logging to service now
                    print()
            ...
            while taskCounter < length:
                ...
                self.keg_log(self.batch_id, "Kegging",
                ...
                if ans in ['Y', 'y', 'yes', 'Yes', 'YES']:
                    while True:  # validation of RFID loop
                        ...
                        if taskRFID in employeeRFID:  # checks to see if RFID is in list
                            ...
                            self.keg_log(self.batch_id, "Kegging",
                                         "Current task: " + taskList[
                                             taskCounter] + " COMPLETED.")  # logging to service now
                            ...
                            self.keg_log(self.batch_id, "Kegging",
                                         "Current kegging tasks status: " + str(
                                             self.task_status))  # logging to service now
                            break
                        else:
